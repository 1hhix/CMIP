...
def run(opts) -> None:
    """
    Run the training or evaluation loop for the constructive model.
    Args:
    ...
    if opts.eval_only:
        ...
        probabilities = np.array(
            [
                1 / (opts.agent_max + 1 - opts.agent_min)
                for _ in range((opts.agent_max + 1 - opts.agent_min))
            ]
        )
        reward_buffer = {}
        if opts.task_sample:
        ...
        for epoch in range(opts.epoch_start, opts.epoch_start + opts.n_epochs):
            ...
            if opts.task_sample and epoch > opts.epoch_start:
                ...
                # normalize
                improve_normalize = (improve - improve.min()) / (
                    improve.max() - improve.min()
                )
                Reg = F.softmax((1 - improve_normalize) * softmax_T, dim=-1)
                probabilities = (
                    opts.p_sample_min + (1 - opts.p_sample_min * num_task) * Reg
                )
