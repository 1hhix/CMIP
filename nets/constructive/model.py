...
class CM(nn.Module):
    ...
    def forward(self, input, return_pi=False):
        ...
        locs_add_depot=torch.cat(
                (depot_locs,
                input[:, 1:, :]),
                1
            )
        self.distance=(locs_add_depot.unsqueeze(1)-locs_add_depot.unsqueeze(2)).norm(dim=-1)
 
        embeddings, _ = self.embedder(self._init_embed(input), agent_num=self.agent_num,distance=None)
    ...
    def _init_embed(self, input):
        ...
        if self.use_circle:
            ...
            positional_embedding = self.cyclic_position_encoding_pattern(seq_length, embedding_dim).to(depot_embedding.device)
    ...
    def _get_log_p(self, fixed, state, selected, normalize=True):
        ...
        log_p, _ = self._one_to_many_logits(query, glimpse_K, glimpse_V, logit_K, mask, selected)
    ...
    def _local_attention(self, batch_size, embed_dim, selected, mask, glimpse_Q, glimpse_K, glimpse_V):
        distance_to_current_nodes = self.distance[torch.arange(self.distance.size(0)), selected, :]
        distance_to_current_nodes[mask.squeeze(1)] = float('inf')
        nearest_nodes_indices = torch.topk(distance_to_current_nodes, self.local_agent_num, dim=1, largest=False)[1]

        local_out = xops.memory_efficient_attention(
            glimpse_Q, 
            glimpse_K[torch.arange(batch_size).unsqueeze(1), nearest_nodes_indices].view(batch_size, self.local_agent_num, self.n_heads, -1), 
            glimpse_V[torch.arange(batch_size).unsqueeze(1), nearest_nodes_indices].view(batch_size, self.local_agent_num, self.n_heads, -1)
    ...
    def _one_to_many_logits(self, query, glimpse_K, glimpse_V, logit_K, mask, selected):
        ...
        mhaout = xops.memory_efficient_attention(
            glimpse_Q, glimpse_K, glimpse_V,# batch,seq_len,embedding
            attn_bias=mask_attn[:,:,:,:last_dim]).view(batch_size, embed_dim)
        if self.use_local:
          # choose the nearest n salesman according to the distance to do the local attention
          if selected != None:
              with torch.no_grad():
                  local_out = self._local_attention(batch_size, embed_dim, selected, mask, glimpse_Q, glimpse_K, glimpse_V)
              if self.use_gate:
                  out = mhaout + local_out * self.gateactivation(self.gatelayer(mhaout))
              else:
        ...
        logits = (
            torch.bmm(glimpse, logit_K.squeeze(1).transpose(-2, -1))
