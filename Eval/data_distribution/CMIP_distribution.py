...
from improvement.policy import (
    eval_dataset, unified_data,
    compute_centroid, compute_distance_matirx,
    Insert_idle, Neighbour_roll
)
...
def main(opts):
    ...
    for param in param_all:
        ...
        # Load dataset 
        dataset_path = f'data/data_distribution/tsp_{opts.distribution}{param[0]}_100_seed3333.pkl'
        print(f'Loading dataset: {dataset_path}')
        dataset = model_CM.problem.make_dataset(filename=dataset_path, num_samples=opts.sample_size, offset=0)
        ...
        action_max_CMIP, reward_CM, _, reward_all_CMIP, action_all_CMIP = unified_data(tours, cost, dataset, opts)
        ...
        do_not_optimize_batch_index, reward_all_CMIP_optimized, action_all_CM = check_idle.cross_insert(dataset)
        ...
        action_all_rolling, reward_all_scroll_optimization, duration_rolling = Rolling.epsilon_greedy_ip_cross(dataset)
        ...
        ref_cost = opt_cost[opts.distribution,(agent_num,graph_size)]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="CMIP Evaluation on different distribution")
    ...
    parser.add_argument('--agent_num', type=int, default=20, help='Initial number of agents')
    parser.add_argument('--graph_size', type=int, default=200, help='Initial graph size')
    parser.add_argument('--constructive_model_path', type=str, default='pretrain/CMIP_constructive/epoch-20.pt', help='Path to constructive model')
    parser.add_argument('--improvement_model_path', type=str, default='pretrain/improvement_model/epoch-99.pt', help='Path to improvement model')
    parser.add_argument('--opt_cost_path', type=str, default='pretrain/distribution_opt.pt', help='Path to optimal cost file')
    parser.add_argument('--epsilon', type=float, default=0.0, help='Epsilon for rolling strategy')
    parser.add_argument('--neighbour_num', type=int, default=5, help='Number of neighbors for IP rolling')
    parser.add_argument('--problem', default="mtsp", type=str, help="problem type")
    parser.add_argument('--distribution', type=str, default='cluster', help='Distribution of mTSP datasets')
