...
def beam_search(*args, **kwargs):
    """
    Perform beam search and return the results.
    Args:
        *args, **kwargs: Arguments for _beam_search
    Returns:
        Tuple of (score, solutions, cost, ids, batch_size)
    """
...
def get_beam_search_results(beams, final_state):
    """
    Extract results from the final beam and state after beam search.
    Args:
        beams: List of BatchBeam objects
        final_state: Final state after search
    Returns:
        Tuple of (score, solutions, cost, ids, batch_size)
    """
...
def _beam_search(state, beam_size: int, propose_expansions=None, keep_states: bool = False):
    """
    Internal beam search loop.
    Args:
        state: Initial state
        beam_size: Beam width
        propose_expansions: Optional function to propose expansions
        keep_states: Whether to keep all states
    Returns:
        Tuple of (beams, final_state)
    """
...
class BatchBeam(NamedTuple):
    ...
    Tensors are flattened across all batch entries.
    """
    ...
    def ids(self) -> torch.Tensor:
        """Return the flat ids for the current beam."""
        return self.state.ids.view(-1)  # Need to flat as state has steps dimension

    def __getitem__(self, key) -> "BatchBeam":
        """Index all beam tensors by the given key (tensor or slice)."""
    ...
    @staticmethod
    def initialize(state) -> "BatchBeam":
        """Initialize a BatchBeam from the given state."""
    ...
    def propose_expansions(self):
        """Propose expansions for the current beam using the state mask."""
    ...
    def expand(self, parent, action, score=None) -> "BatchBeam":
        """Expand the beam with the given parent and action."""
        return self._replace(
    ...
    def topk(self, k: int) -> "BatchBeam":
        """Select the top-k entries in the beam by score."""
        idx_topk = segment_topk_idx(self.score, k, self.ids)
        return self[idx_topk]

    def all_finished(self) -> bool:
        """Check if all entries in the beam are finished."""
        return self.state.all_finished()

    def cpu(self) -> "BatchBeam":
        """Move the beam to CPU."""
        return self.to(torch.device("cpu"))

    def to(self, device) -> "BatchBeam":
        """Move the beam to the specified device."""
    ...
    def clear_state(self) -> "BatchBeam":
        """Clear the state from the beam (for memory efficiency)."""
        return self._replace(state=None)

    def size(self) -> int:
        """Return the number of entries in the beam."""
...
def segment_topk_idx(x: torch.Tensor, k: int, ids: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        x: 1D tensor of scores
        k: Number of top elements per segment
        ids: 1D tensor of segment ids
    Returns:
        Indices of the top-k elements per segment
    """
    assert x.dim() == 1
    assert ids.dim() == 1
