...
def train_epoch(model, optimizer, lr_scheduler, epoch, problem, opts, 
                save_checkpoint=True,probabilities=None,best_reward=None):
    ...
    if opts.train_on_different_node:
      ...
      training_dataset = problem.make_dataset(
          size=graph_size, num_samples=opts.epoch_size, distribution=opts.data_distribution,opts=opts)
    ...
    for batch_id, batch in enumerate(tqdm(training_dataloader, disable=opts.no_progress_bar,desc=f'Epoch:{epoch}')):
        if opts.task_sample:
            ...
            if opts.train_on_different_node:
                ...
                graph_size=random.sample(range(opts.graph_min, opts.graph_max + 1), 1)[0]
...
def train_batch(
    ...
    best_reward[model.agent_num-opts.agent_min]=best_cost_mean if best_cost_mean<best_reward[model.agent_num-opts.agent_min] else best_reward[model.agent_num-opts.agent_min] 
